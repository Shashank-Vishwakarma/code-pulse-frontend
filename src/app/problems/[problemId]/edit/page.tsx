...
export default function EditProblemPage() {
    ...
    const {data: challenge} = useGetQuestionByIdQuery(params.problemId as string);
    ...
    useEffect(()=>{
        ...
        setDifficulty(challenge?.data?.difficulty || "");
        setTopics(challenge?.data?.tags || []);
        setTestCases(challenge?.data?.testCases || []);
        setHints(challenge?.data?.hints || []);
        setCompanies(challenge?.data?.companies || []);
        setCodeSnippets(challenge?.data?.codeSnippets || []);

        form.reset({
            title: challenge?.data?.title || "",
            description: challenge?.data?.description || "",
            difficulty: challenge?.data?.difficulty || "",
            testCases: challenge?.data?.testCases || [{input: "", output: "", explanation: ""}],
            tags: challenge?.data?.tags || [],
            hints: challenge?.data?.hints || [],
            companies: challenge?.data?.companies || [],
            codeSnippets: challenge?.data?.codeSnippets || [
                {language: "javascript", code: ""},
                {language: "python", code: ""}
            ]
        })
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            const payload = await updateQuestion({id: params?.problemId as string, data: values}).unwrap();
            if (payload) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-600 to-blue-900 container mx-auto p-4">
            ...
            <Card className='mt-20'>
                <CardHeader>
                    <CardTitle>Update Problem</CardTitle>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                                    ...
                                    <div key={index} className="flex gap-4 items-start">
                                        <div className="flex-1 space-y-2">
                                            <Input
                                                ...
                                                onChange={(e) => {
                                                    const updatedTestCases = testCases.map((testCase, i)=>(
                                                        i==index ? {...testCase, input: e.target.value} : testCase
                                                    ))
                                                    setTestCases(updatedTestCases);
                                                    form.setValue("testCases", updatedTestCases);
                                                }}
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => {
                                                    const updatedTestCases = testCases.map((testCase, i)=>(
                                                        i==index ? {...testCase, output: e.target.value} : testCase
                                                    ))
                                                    setTestCases(updatedTestCases);
                                                    form.setValue("testCases", updatedTestCases);
                                                }}
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => {
                                                    const updatedTestCases = testCases.map((testCase, i)=>(
                                                        i==index ? {...testCase, explanation: e.target.value} : testCase
                                                    ))
                                                    setTestCases(updatedTestCases);
                                                    form.setValue("testCases", updatedTestCases);
                                                }}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className='flex flex-col gap-2'>
                                    ...
                                    <Textarea
                                        ...
                                        value={codeSnippets?.length > 0 && codeSnippets[0]?.code || ""}
                                        onChange={(e)=>{
                                ...
                                <div className='flex flex-col gap-2'>
                                    ...
                                    <Textarea
                                        ...
                                        className="font-mono h-32"
                                        value={codeSnippets?.length > 0 && codeSnippets[1]?.code || ""}
                                        onChange={(e)=>{
                                            const newCodeSnippets = [
                                                codeSnippets[0],
                            ...
                            <Button type="submit" className="w-full">
                                {
                                    isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Update Problem"
                                }
                            </Button>
                        </form>
                    </Form>
